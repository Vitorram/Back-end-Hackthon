...
def create_equipment(db: Session, data: EquipmentCreate, current_user: dict):
    ...
    if not _can_manage_equipment(current_user, data.escola_atual_id):
        raise HTTPException(status_code=403, detail="Sem permissao para criar equipamento")

    if data.patrimonio and equipment_repository.get_equipment_by_patrimonio(db, data.patrimonio):
